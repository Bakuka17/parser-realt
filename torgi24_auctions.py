...
def parse():
    items=[]; seen=set()
    html=A.fetch("https://torgi24.by/nedvizhimost")
    if not html: print("список не скачался"); return items
    links=list(dict.fromkeys(re.findall(r'href="((?:https?://torgi24\.by/)?node/\d+)"', html)))
    print(f"ссылок: {len(links)}")
    for link in links:
        full=link if link.startswith("http") else "https://torgi24.by/"+link
        nu=A.norm_url(full)
        ...
        if not d: continue
        text=get_text(d)
        it=A.blank_item("torgi24.by"); it["Ссылка"]=nu; it["Тип торгов"]="Аукцион"
...
if __name__=="__main__":
    res=parse(); print(f"[torgi24] лотов: {len(res)}")
    if res:
    ...
    A.write_excel(res, Path("auctions_torgi24.xlsx"))
