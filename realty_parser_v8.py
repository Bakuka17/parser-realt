...
# маркеры ХВОСТА-контакта/рекламы (срезаем от первого). НЕ берём «агентств» — оно часто
# в ЛЕГИТ-преамбуле («Арендатор не оплачивает услуги агентства. Предлагаем…») и резало бы текст.
_DESC_CUT = re.compile(
    r"(?:звонит|тел\.|тел:|телефон|\+?375|\b8\s*\(?\s*0\d|риелтор|риэлтор|"
    r"подробнее\s+по\s+тел|все\s+объект\w*\s+(?:на|компании)|наш\s+сайт|на\s+сайте|"
    r"whatsapp|viber|вайбер)",
    re.I,
)


def clean_description(text: str, limit: int = 500) -> str:
    """Описание объекта для таблицы обзвона: схлопнуть пробелы; срезать ХВОСТ от первого
    маркера контактов/рекламы (агентство/звоните/тел./+375/риелтор/«все объекты»…), сохранив
    точку конца предложения; обрезать до limit по границе слова. Если маркер в самом начале —
    вернуть нормализованный текст целиком (не пустоту)."""
    if not text:
        return ""
    t = re.sub(r"\s+", " ", str(text)).strip()
    m = _DESC_CUT.search(t)
    if m and m.start() > 0:
