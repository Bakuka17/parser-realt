...
def main():
    ...
    if total == 0:
        # Гео-блок под VPN (или belretail лёг): 0 компаний = НЕ данные, а отказ.
        # Старый файл не трогаем — 09.07.2026 именно такая запись стёрла все компании.
        print("\n✖ Собрано 0 компаний (VPN включён? belretail отвечает только бел. IP) — "
              f"существующий {OUT.name} НЕ перезаписан.")
        return

    for col, w in zip("ABCD", (24, 34, 28, 52), strict=False):
        ws.column_dimensions[col].width = w
    ws.freeze_panes = "A2"
    tmp = OUT.with_suffix(".xlsx.tmp")   # атомарно: обрыв посреди записи не бьёт боевой файл
    wb.save(tmp)
    os.replace(tmp, OUT)
